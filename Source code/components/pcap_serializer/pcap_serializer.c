...
void pcap_serializer_append_frame(const uint8_t *buffer, unsigned size, unsigned ts_usec){
    ...
    // Limit PCAP size to prevent heap fragmentation scaling issues on long attacks (~100KB max)
    if(pcap_size + size > (80 * 1024)) {
        ESP_LOGV(TAG, "PCAP buffer full (%d bytes). Dropping extra frames to prevent RAM crush.", pcap_size);
        return;
    }
