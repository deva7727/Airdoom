...
static void attack_timeout(void* arg){
    ...
    switch(attack_status.type) {
        ...
        case ATTACK_TYPE_DOS:
            ...
            attack_dos_stop();
            break;
        case ATTACK_TYPE_BEACONSPAM:
            ESP_LOGI(TAG, "Abort Beacon Spam attack...");
            attack_beaconspam_stop();
            break;
        case ATTACK_TYPE_MASSDEAUTH:
            ESP_LOGI(TAG, "Abort Mass Deauther attack...");
            attack_massdeauth_stop();
            break;
        default:
...
static void attack_request_handler(void *args, esp_event_base_t event_base, int32_t event_id, void *event_data) {
    ...
    if(attack_config.ap_record == NULL && attack_config.type != ATTACK_TYPE_MASSDEAUTH && attack_config.type != ATTACK_TYPE_BEACONSPAM){
        ESP_LOGE(TAG, "NPE: No attack_config.ap_record!");
        return;
    }
    // Ensure timer is stopped before starting it to avoid ESP_ERR_INVALID_STATE crash
    ...
    switch(attack_config.type) {
        ...
        case ATTACK_TYPE_HANDSHAKE:
            ...
            break;
        case ATTACK_TYPE_PASSIVE:
            attack_passive_start(&attack_config);
            break;
        case ATTACK_TYPE_DOS:
            attack_dos_start(&attack_config);
            break;
        case ATTACK_TYPE_BEACONSPAM:
            attack_beaconspam_start(&attack_config);
            break;
        case ATTACK_TYPE_MASSDEAUTH:
            attack_massdeauth_start(&attack_config);
            break;
        default:
