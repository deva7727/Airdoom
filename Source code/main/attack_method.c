...
void attack_method_broadcast(const wifi_ap_record_t *ap_record, unsigned period_ms){
    ...
    // Trigger the first burst IMMEDIATELY so the user doesn't have to wait for the first interval
    timer_send_deauth_frame((void *) ap_record);
}

void attack_method_broadcast_stop(){
    esp_err_t err = esp_timer_stop(deauth_timer_handle);
    if(err != ESP_OK && err != ESP_ERR_INVALID_STATE) {
        ESP_LOGE(TAG, "Timer stop failed: %s", esp_err_to_name(err));
    }
